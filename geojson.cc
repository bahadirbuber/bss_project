...
int geojson_t::parse_feature(JsonValue value)
{
  ...
  for (JsonNode *obj = object.toNode(); obj != nullptr; obj = obj->next)
  {
    ...
    else if (std::string(obj->key).compare("properties") == 0)
    {
      ...
      {
        //get counrtry name
        if (std::string(prp->key) == "country" || std::string(prp->key) == "ADMIN")
        {
          assert(prp->value.getTag() == JSON_STRING);
          feature.m_country = prp->value.toString();
        }
          //get city name
        if (std::string(prp->key) == "city")
        {
          assert(prp->value.getTag() == JSON_STRING);
          feature.m_city = prp->value.toString();
        }
        //get iso3
        if (std::string(prp->key) == "iso3" || std::string(prp->key) == "ISO_A3")
        {
          assert(prp->value.getTag() == JSON_STRING);
          feature.m_iso3 = prp->value.toString();
        }
      }
    }
    else if (std::string(obj->key) == "geometry")
    {
      assert(obj->value.getTag() == JSON_OBJECT);
...
int geojson_t::parse_geometry(JsonValue value, feature_t &feature)
{
  ...
  for (JsonNode *node = value.toNode(); node != nullptr; node = node->next)
  {
    if (std::string(node->key) == "type")
    {
      assert(node->value.getTag() == JSON_STRING);
      str_geometry_type = node->value.toString();
    }
    else if (std::string(node->key) == "coordinates")
    {
      assert(node->value.getTag() == JSON_ARRAY);
      //str_geometry_type = node->value.toString();
        if(str_geometry_type.empty()) str_geometry_type = "Point";
        if (str_geometry_type == "Point")
      {
        /////////////////////////////////////////////////////////////////////////////////////////////////////
      ...
      if (str_geometry_type == "Polygon")
      {
        assert(node->value.getTag() == JSON_ARRAY);
        parse_coordinates(node->value, str_geometry_type, feature);
      }
      if (str_geometry_type == "MultiPolygon")
      {
        assert(node->value.getTag() == JSON_ARRAY);
        parse_coordinates(node->value, str_geometry_type, feature);
      }
